<?php
/* $Id: Worksheet.php 6436 2011-05-12 05:53:51Z brucelyc $
...
require_once $SPREADSHEET_PATH.'Excel/Writer/Parser.php';
require_once $SPREADSHEET_PATH.'Excel/Writer/BIFFwriter.php';
    ...
    function writeStringBIFF8($row, $col, $str, $format = null)
    {
        if ($this->_input_encoding != '')
		{
            if ($this->_input_encoding != 'UTF-16LE')
				$str = sfs_iconv($this->_input_encoding, 'UTF-16LE', $str);
...
// trans &#xxxxx to UTF-16LE
function sfs_iconv($c1="",$c2="",$str="") {

	$arr=explode("&#",$str);
	if (count($arr)>1) {
		$all_str=iconv($c1,$c2,$arr[0]);
		$chg=0;
		for($i=1;$i<count($arr);$i++) {
			...
			if (count($arr2)>1) {
				$s="%0".strlen($arr2[0])."d";
				$d=intval($arr2[0]);
				if (sprintf($s,$d)==$arr2[0]) {
					$all_str.=pack("v",$d);
					$chg++;
					for($j=1;$j<count($arr2);$j++) {
						if ($j>1) $all_str.=iconv($c1,$c2,";");
						$all_str.=iconv($c1,$c2,$arr2[$j]);
					}
				} else {
					$all_str.=iconv($c1,$c2,"&#".$arr[$i]);
				}
			} else {
				$all_str.=iconv($c1,$c2,"&#".$arr[$i]);
			}
