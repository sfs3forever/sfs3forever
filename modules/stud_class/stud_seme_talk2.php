...
<tr id="nor_form6">
	...
	<?php 
		...
		$im.="</select><font size=1 color='red'> (選項由模組變數中設定!)</font>";
...
<?php 
	...
	$sel->do_select();
	echo "<b>$stud_name</b> ";
?> 輔導訪談記錄 , <a href="stud_talk_detail.php?student_sn=<?php echo $student_sn; ?>" target="show_all" >列出全部記錄</a>

</FORM>
<form name="myform1" method="post" action="<?php echo $_SERVER['SCRIPT_NAME'] ?>">
...
while (!$recordSet->EOF) {
	...
	if($sel_this_year == $this_year || $old_year_is_edit) {
		echo " <a href=\"{$_SERVER['SCRIPT_NAME']}?do_key=edit&sst_id=$sst_id\"";
		$ol->pover($sst_name,$sst_memo);
		echo ">檢視 / 修改</a>&nbsp;|&nbsp;<a href=\"{$_SERVER['SCRIPT_NAME']}?do_key=delete&sst_id=$sst_id&stud_id=$stud_id\" onClick=\"return confirm('確定刪除?');\">刪除</a>";
	}
	else
	{
		echo " <a ";
