...
if ($do_key ==  $newBtn) {
	...
	if ($seme_year_seme =='')
		...
		$query  = "SELECT  a.stud_id  FROM  stud_base a,stud_seme b where a.student_sn=b.student_sn  and (a.stud_study_cond=0 or a.stud_study_cond=5) and  b.seme_year_seme='$c_curr_seme' and b.seme_class='$seme_class'   ";   //SQL 命令		
...
while (!$recordSet->EOF) {
	...
	if($seme_year_seme == $sel_seme_year_seme) {
		...
		if ($teacher_sn == $_SESSION[session_tea_sn]) {
			...
			//echo "ey=delete&st_id=$st_id&stud_id=$stud_id\" onClick=\"return confirm('確定刪除?');\">刪除</a>";			
		}
		else {
				echo " <a href=\"{$_SERVER['SCRIPT_NAME']}?do_key=edit&st_id=$st_id\"   >檢視</a> "  ;
		}
