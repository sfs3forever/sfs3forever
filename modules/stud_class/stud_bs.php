...
// $Id: stud_bs.php 6986 2012-10-31 06:39:15Z infodaes $
...
//建立左邊選單   
	...
	$grid1->sql_str = "select a.student_sn,a.stud_id,a.stud_name,a.stud_sex,b.seme_num as sit_num from stud_base a,stud_seme b where a.student_sn=b.student_sn and a.stud_study_cond=0 and b.seme_year_seme='$c_curr_seme' and b.seme_class='$class_num' order by b.seme_num ";   //SQL 命令
	$grid1->do_query(); //執行命令   
	...
	while (!$recordSet->EOF) {
		...
		if ($bs_id) {
			...
			if ($ckey==$editModeBtn && $bs_id) { //修改模式
				...
				echo "<td> <a href=\"{$_SERVER['SCRIPT_NAME']}?key=delete&bs_id=$bs_id&ckey=$ckey&$linkstr\" onClick=\"return confirm('確定刪除 ".$bs_name." 記錄?');\">刪除</a></td>";
			}
			else {
