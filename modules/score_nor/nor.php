...
if(($year_seme)&&($year_name)&&($me)){
	...
	$main="	<a href={$_SERVER['PHP_SELF']}?act=print&year_seme=$year_seme&year_name=$year_name&me=$me>友善列印</a>
	...
	for ($m=0;$m<count($stud_sn);$m++){
		...
		$stud_abs[$id][7]=$months;
		for ($i=$month_start;$i<=$month_end;$i++) {
			$j=$i;
			if ($i>12) $j=$i-12;
			if ($abs_month[$j]==1) $stud_abs[$id][7]--;
		}
		if ($a_score!="1") $stud_abs[$id][7]=($stud_abs[$id][7]==$months)?"5":"0";
		//取得出缺席值
		...
		$stud_abs[$id][0]=$stud_abs[$id][7]-floor($stud_abs[$id][3]/2+$stud_abs[$id][2]/$sl_days+$stud_abs[$id][1]/$cl_days+$stud_abs[$id][4]/4);
		$abs_score.="<td align='center'>".$stud_abs[$id][7]."</td>";
		...
		$main.="
			...
			<td align='right'>$site_num &nbsp;</td>
               		...
               		";
               	if ($act) {
			$smarty_stud_name[$site_num]=$stud_name;
			$smarty_stud_id[$site_num]=$id;
			$smarty_abs[$site_num]=$stud_abs[$id];
			$smarty_nor[$site_num]=$stud_score[$id];
			$smarty_rew[$site_num]=$stud_rew[$id];
			$smarty_total[$site_num]=$nor_total;
			$smarty_memo[$site_num]=stripslashes($ss_memo);
		}
...
$help_text="如果勤惰及獎懲內容不正確，建議您先<a href='cal.php'>重新統計日常成績</a>。";
if (!$act) {
	echo $main."</tr></table></tr></table>".help($help_text);
	foot();
} else {
	$smarty->assign("year",$sel_year);
	$smarty->assign("seme",$sel_seme);
	$smarty->assign("year_name",$year_name);
	$smarty->assign("me",sprintf("%02d",$me));
	$smarty->assign("school",get_school_base());
	$smarty->assign("class_year",$class_year);
	$smarty->assign("class_name",class_name());
	$smarty->assign("smarty_stud_name",$smarty_stud_name);
	$smarty->assign("smarty_stud_id",$smarty_stud_id);
	$smarty->assign("smarty_abs",$smarty_abs);
	$smarty->assign("smarty_nor",$smarty_nor);
	$smarty->assign("smarty_rew",$smarty_rew);
	$smarty->assign("smarty_total",$smarty_total);
	$smarty->assign("smarty_memo",$smarty_memo);
	$smarty->assign("max_site",$site_num);
