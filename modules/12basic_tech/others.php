...
if($_POST['act']=='確定修改'){
	$edit_other=serialize($_POST['edit_other']);
	$sql="UPDATE 12basic_tech SET others_item='$edit_other',others_memo='{$_POST['edit_memo']}' WHERE academic_year=$work_year AND student_sn=$edit_sn AND editable='1'";
...
if($_POST['act']=='批次更新'){
	foreach($_POST[batch] as $student_sn=>$data) {
		$edit_other=serialize($data['edit_other']);
		$sql="UPDATE 12basic_tech SET others_item='$edit_other',others_memo='{$data['edit_memo']}' WHERE academic_year=$work_year AND student_sn=$student_sn AND editable='1'";
		$res=$CONN->Execute($sql) or user_error("讀取失敗！<br>$sql",256);
	}
	$edit_sn=0;
	$_POST['batch']='';
...
if($stud_class)
{
	...
	//取得指定學年已經開列的學生其他分數	
	$other_array=get_student_others($work_year);
	...
	$studentdata="<tr align='center' bgcolor='#ff8888' $java_script><td width=80>學號</td><td width=50>座號</td><td width=120>姓名</td><td width=$pic_width>大頭照</td><td>其他比序項目紀錄</td><td>備註</td>";
/*
echo '<pre>';
print_r($other_array);
echo '</pre>';
*/
	while(list($student_sn,$seme_num,$stud_name,$stud_sex,$stud_id,$stud_study_year)=$recordSet->FetchRow()) {
		...
		$other='';
		foreach($other_array[$student_sn]['item'] as $key=>$value){
			if($value) $other.="<li>{$others_array[$key]['name']}：<b><font color='blue'>{$others_array[$key]['items'][$value]}</font></b></li>";		
		}
		$memo=$other_array[$student_sn]['memo'];
		$action='';
		$java_script='';
		...
		if($_POST['batch']){
			if(array_key_exists($student_sn,$editable_sn_array) and array_key_exists($student_sn,$listed)){
				$other='';
				foreach($others_array as $key=>$data){
					$my_level="<select name='batch[$student_sn][edit_other][$key]'><option value='' selected>";
					foreach($data['items'] as $id=>$value){
						$selected=($id==$other_array[$student_sn]['item'][$key])?'selected':'';
						$my_level.="<option value='$id' $selected>$value</option>";
					}
					$my_level.="</select>";
					$other.="<li>{$data['name']}: $my_level</li>";
				}				
			...
			if($student_sn==$edit_sn){
				...
				$other='';
				foreach($others_array as $key=>$data){
					$my_level="<select name='edit_other[$key]'><option value='' selected>";
					foreach($data['items'] as $id=>$value){
						$selected=($id==$other_array[$student_sn]['item'][$key])?'selected':'';
						$my_level.="<option value='$id' $selected>$value</option>";
					}
				...
				//動作按鈕
				$action="<input type='submit' name='act' value='確定修改' onclick='return confirm(\"確定要修改 $stud_name 的其他比序項目紀錄?\")'> <input type='submit' name='act' value='取消' onclick='document.myform.edit_sn.value=0;'>";		
			} else {
		...
		$studentdata.="<tr align='center' bgcolor='$stud_sex_color' $java_script><td>$stud_id</td><td>$seme_num</td><td>$stud_name</td><td>$my_pic</td><td bgcolor='$bgcolor_other' align='left'>$other</td><td align='left'>$memo $action</td></tr>";
	}
