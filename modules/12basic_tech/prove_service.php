		...
		foreach($data as $key=>$value){
			if($key<>'data' and $key<>'memo'){
				foreach($value as $leader_name){
					if($leader_name) $leader[$student_sn][$seme_key].=$leader_name.'<br>';
				}
			}
		}
	}
...
//社團	
$sql="SELECT seme_year_seme,association_name,student_sn,stud_post FROM association WHERE student_sn IN ($sn_list)";
$res=$CONN->Execute($sql) or user_error("讀取失敗！<br>$sql",256);
while(!$res->EOF)
{
		$student_sn=$res->fields['student_sn'];
		...
		$leader[$student_sn][$seme_key].=$res->fields[1].$res->fields['stud_post'].'<br>';
		$res->MoveNext();
}
...
foreach($all as $class_id=>$students){
	...
	echo "<h2>{$semester_year}學年度五專免試入學<br>超額比序多元學習表現之「服務學習-幹部、小老師」證明單</h2>";
	...
	echo "<table border='2' cellpadding='3' cellspacing='0' style='font-size:11px; border-collapse: collapse' bordercolor='#111111' id='AutoNumber1' width='100%'>
			<tr align='center'><td rowspan=2>座號</td><td rowspan=2>姓名</td><td colspan=6>服務表現項目</td><td rowspan=2>備註</td></tr>
			<tr align='center'><td>7上</td><td>7下</td><td>8上</td><td>8下</td><td>9上</td><td>9下</td></tr>";
	foreach($students as $student_sn=>$data) 	
		echo "<tr align='center'><td>{$data['seme_num']}</td><td>{$data['stud_name']}</td><td>{$leader[$student_sn]['7-1']}</td><td>{$leader[$student_sn]['7-2']}</td><td>{$leader[$student_sn]['8-1']}</td><td>{$leader[$student_sn]['8-2']}</td><td>{$leader[$student_sn]['9-1']}</td><td>{$leader[$student_sn]['9-2']}</td><td>{$move[$student_sn]}{$leader[$student_sn]['memo']}</td></tr>";
