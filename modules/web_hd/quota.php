...
if($act=="save"){//儲存設定
	foreach ($size_arr as $ts => $sz){
		...
		$sql="update hd_quota set size='$sz' , many='{$many_arr[$ts]}' where teacher_sn='$ts'  ";
...
while(!$rs->EOF){
	...
	$sql2="select * from hd_quota where teacher_sn='{$teacher_sn[$i]}' and enable=1";
	$rs2=$CONN->Execute($sql2) or trigger_error($sql2,256);

	//給預設值
	if(!$rs2->fields['size'] && !$rs2->fields['many']) $CONN->Execute("insert into hd_quota (teacher_sn,size,many,enable) values('$teacher_sn[$i]','20','200','1')");
