...
if($_POST['act']){
	...
	switch($_POST['act']){
		...
		case 'EXCEL_SCORE':
			...
			$x->setBorderStyle(1);
			$x->addSheet($school_id);
			$x->items[0]=array('學生姓名','身分證統一編號','出生年(民國年)','出生月','出生日','畢業資格','經濟弱勢','均衡學習','服務表現','品德表現','競賽表現','體適能','適性發展');
			break;	
		case 'HTML_SCORE':
			$main="<table border='2' cellpadding='3' cellspacing='0' style='border-collapse: collapse' bordercolor='#111111' id='AutoNumber1'>
				<tr bgcolor='#ffcccc'><td>學生姓名</td><td>身分證統一編號</td><td>出生年(民國年)</td><td>出生月</td><td>出生日</td><td>畢業資格</td><td>經濟弱勢</td><td>均衡學習</td><td>服務表現</td><td>品德表現</td><td>競賽表現</td><td>體適能</td><td>適性發展</td>";
			break;	
	}
			...
			*/
			...
			switch($_POST['act']){
				...
				case 'EXCEL_SCORE':
					...
					break;
				case 'HTML_SCORE':
					$main.="<tr align='center'><td>$stud_name</td><td>$stud_person_id</td><td>$birth_year</td><td>$birth_month</td><td>$birth_day</td><td>$score_graduate</td><td>$score_disadvantage</td><td>$score_balance</td><td>$score_service</td><td>$score_fault</td><td>$score_competetion</td><td>$score_fitness</td><td>$score_personality</td>";
					break;
			}
		}
		$recordSet->MoveNext();
