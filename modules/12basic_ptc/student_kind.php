...
if($_POST['act']=='修改'){
	$sql="UPDATE 12basic_ptc SET kind_id='{$_POST[kind_id]}',disability_id='{$_POST[disability_id]}',free_id='{$_POST[free_id]}',id_memo='{$_POST[id_memo]}' WHERE academic_year=$work_year AND student_sn=$edit_sn AND editable='1'";
...
if($_POST['act']=='批次更新'){
	foreach($_POST['batch'] as $student_sn=>$data) {
		$sql="UPDATE 12basic_ptc SET kind_id='{$data[kind_id]}',disability_id='{$data[disability_id]}',free_id='{$data[free_id]}',id_memo='{$data[id_memo]}' WHERE academic_year=$work_year AND student_sn=$student_sn AND editable='1'";
		$res=$CONN->Execute($sql) or user_error("更新失敗！<br>$sql",256);
	}
	$edit_sn=0;
	$_POST['batch']='';
...
if($stud_class)
{
	...
	//抓取既有身份對照表
	$sql="SELECT kind_data,disability_data,free_data FROM 12basic_kind_ptc WHERE year_seme='$work_year_seme'";
	...
	$studentdata="<tr align='center' bgcolor='#ff8888' $java_script><td width=80>學號</td><td width=50>班級</td><td width=50>座號</td><td width=120>姓名</td><td width=$pic_width>大頭照</td><td>SFS3內註記的身份類別</td><td>報名身份</td><td>身心障礙</td><td>低收失業</td><td>備註</td>";
