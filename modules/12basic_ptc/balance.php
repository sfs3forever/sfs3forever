...
if($_POST['act']=='統計本年度所有開列學生均衡學習的積分'){
	...
	foreach($sn_array as $key=>$student_sn){
		$sql="update 12basic_ptc set score_balance_health='{$score_balance_array[$student_sn]['health']['bonus']}',score_balance_art='{$score_balance_array[$student_sn]['art']['bonus']}',score_balance_complex='{$score_balance_array[$student_sn]['complex']['bonus']}' where academic_year=$work_year AND student_sn=$student_sn AND editable='1'";
		$res=$CONN->Execute($sql) or user_error("讀取失敗！<br>$sql",256);
	}
};

if($_POST['act']=='清除所有開列學生均衡學習的積分'){
	$sql="update 12basic_ptc set score_balance_health=NULL,score_balance_art=NULL,score_balance_complex=NULL where academic_year=$work_year AND editable='1'";
	$res=$CONN->Execute($sql) or user_error("讀取失敗！<br>$sql",256);
};
...
if($_POST['act']=='確定修改'){
	...
	//更新
	$sql="update 12basic_ptc set score_balance_health={$_POST['score_balance_health']},score_balance_art={$_POST['score_balance_art']},score_balance_complex={$_POST['score_balance_complex']},balance_memo='{$_POST['memo']}'
...
if($_POST['act']=='批次更新'){
	foreach($_POST['batch'] as $student_sn=>$data) {
		//決定高限
		$score_balance_health=min($data['score_balance_health'],$balance_score);
		$score_balance_art=min($data['score_balance_art'],$balance_score);
		$score_balance_complex=min($data['score_balance_complex'],$balance_score);
		$sql="update 12basic_ptc set score_balance_health='$score_balance_health',score_balance_art='$score_balance_art',score_balance_complex='$score_balance_complex',balance_memo='{$data['memo']}'
...
if($work_year==$academic_year and $editable_sn_array) $tool_icon.=" <input type='submit' value='統計本年度所有開列學生均衡學習的積分' name='act' onclick='return confirm(\"確定要重新統計本年度所有開列學生均衡學習的積分?\")'>
...
if($stud_class)
{
	...
	$studentdata="<tr align='center' bgcolor='#ff8888' $java_script><td width=80>學號</td><td width=50>座號</td><td width=120>姓名</td><td width=$pic_width>大頭照</td><td width=50>健體</td><td width=50>藝文</td><td width=50>綜合</td><td>積分統計</td><td>備註</td>";
	while(list($student_sn,$seme_num,$stud_name,$stud_sex,$stud_id,$stud_study_year)=$recordSet->FetchRow()) {
		...
		$stud_sex_color=$listed[$student_sn]?$stud_sex_color:'#aaaaaa';
		$stud_sex_color=$editable_sn_array[$student_sn]?$stud_sex_color:$uneditable_bgcolor;
		...
		if($_POST['batch']){
			if($editable_sn_array[$student_sn] and $listed[$student_sn]){
				$score_balance_health="<input type='text' name='batch[$student_sn][score_balance_health]' value='$score_balance_health' size=5>";
				$score_balance_art="<input type='text' name='batch[$student_sn][score_balance_art]' value='$score_balance_art' size=5>";
				$score_balance_complex="<input type='text' name='batch[$student_sn][score_balance_complex]' value='$score_balance_complex' size=5>";
			...
			if($student_sn==$edit_sn){
				...
				if($listed[$student_sn]){
					$editable=$editable_sn_array[$student_sn]?1:0;
	...
	if($_POST['batch']) $studentdata.="<tr align='center'><td colspan=10><input type='submit' name='act' value='批次更新' onclick='return confirm(\"確定要修改本班學生所有的均衡學習積分?\")'> <input type='submit' name='act' value='取消'></td></tr>";
}
