...
//if ($_POST['year_name']) {
	...
	if ($res) {		
		...
		$smarty->assign("teacher_array",$teacher_array);
		...
		//取課程設定資料
		$query="select ss_id,scope_id,subject_id from score_ss where year='$sel_year' and semester='$sel_seme' and enable=1";
		$res=$CONN->Execute($query);
		while(!$res->EOF) {
...
$smarty->assign("module_name","百分成績檢查"); 
...
$smarty->display("score_manage_new_check100.tpl");
?>
