<?php
//$Id: mgr_special.php 8769 2016-01-13 14:16:55Z qfon $
include "config.php";
//認證
sfs_check();
//if ($_POST){
...
function stud_list($mid,$item) {
		global $CONN,$sportname,$itemkind,$sportclass,$PHP_SELF;
...
<TABLE border=0 width=100%><FORM METHOD=POST ACTION='<?=$PHP_SELF?>' name='f1'><TR><TD colspan=2><img src='images/12.gif'><B><?=$ITEM_NAME?>&nbsp;參賽單位分組列表</B>
</TD></TR>
<TR><TD colspan=2><img src='images/12.gif'><B>項目資訊</B>
己報名<?=$rs->RecordCount()?>隊，每組<?=$limit?>隊，取<?=$item_arr[0][passera]?>名，
...
for ($a=1;$a<=$all_col ;$a++){
	...
	if ( $arr[$i][sportorder] > $LimtA && $arr[$i][sportorder] < $LimtB ) {
		if ($_GET[txt]=='open') {
			$tmp_str.="<INPUT TYPE='text' NAME='chstu[".$arr[$i][id]."_".$item."]' size=3 value='".$arr[$i][sportorder]."' $dd onfocus=\"this.select();return false ;\" onkeydown=\"moveit2(this,event);\" class=bur><B style='color:red'>".$arr[$i][cname]."-".$arr[$i][kgp]."</B>代表隊\n";
			}
		else {
			$tmp_str.="<INPUT TYPE='checkbox' NAME='dstu[".$arr[$i][id]."_".$item."]' value='".$arr[$i][cname]."' $dd>(<B style='color:blue'>".$arr[$i][sportorder]."</B>)<B style='color:red'>".$arr[$i][cname]."-".$arr[$i][kgp]."</B>代表隊\n";
			}
...
################	未分組處理		######################
	...
	for($i=0; $i<$rs->RecordCount(); $i++) {
		...
		if ($_GET[txt]=='open') {
			...
			$tmp_str.="<INPUT TYPE='checkbox' NAME='astu[".$arr[$i][id]."_".$item."]' value='".$arr[$i][cname]."'><B style='color:blue'>".$arr[$i][cname]."-".$arr[$i][kgp]."</B>代表隊\n";}
...
//	(
		...
		"&nbsp;(報名組數: $Nu1 編號組數: $Nu2 檢錄組數: $Nu3 成績組數: $Nu4)</option>\n";
