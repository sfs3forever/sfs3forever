...
// $Id: stud_psy_test.php 6494 2011-08-29 06:53:17Z infodaes $
...
<?php
	//建立左邊選單   
	// 2012/10/25 by smallduh 鎖定本學期 ===========================
	//$class_seme_p = get_class_seme(); //學年度	
	$class_seme_p[$this_seme_year_seme] = substr($this_seme_year_seme,0,3)."學年第".substr($this_seme_year_seme,-1)."學期"; //學年度	
...
<?php 
	...
	if ($old_year_is_edit) {
		...
		//2012-10-25 不允許改其他學期資料
		$sel->arr = array($this_seme_year_seme=>substr($this_seme_year_seme,0,3)."學年第".substr($this_seme_year_seme,-1)."學期");
		$sel->do_select();
		echo sprintf(" --%s (%s)",$stud_name,$student_sn);
	}
	else   	
		echo sprintf("%d學年第%d學期 %s--%s (%s)",substr($c_curr_seme,1,2),substr($c_curr_seme,-1),$class_list_p[$c_curr_seme],$stud_name,$student_sn);
