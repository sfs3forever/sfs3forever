...
$sql= "select distinct year_seme from sms_mitake_record order by year_seme desc";
...
//抓取紀錄
$sql="SELECT teacher_sn,count(*) AS counter,MAX(ask_time) AS recenttime FROM sms_mitake_record WHERE year_seme='$year_seme' AND statuscode='4' AND not private ORDER BY counter";
...
$main.="<table border='2' cellpadding='5' cellspacing='0' style='border-collapse: collapse' bordercolor='#111111' id='AutoNumber1' width='100%'>
		<tr bgcolor='#ccffff' align='center'><td>NO.</td><td>教師代號</td><td>教師姓名</td><td>送達手機總筆數</td><td>最後發送時間</td></tr>";
while(list($teacher_sn,$counter,$recenttime)=$recordSet->FetchRow()) {
	$num=sprintf('%03d',$recordSet->currentrow());
	$main.="<tr align='center'><td>$num</td><td>$teacher_sn</td><td>{$teacher_array[$teacher_sn]}</td><td>$counter</td><td>$recenttime</td></tr>";
}
