<?php
//$Id: function.php 8783 2016-01-19 05:33:27Z qfon $
...
function get_school_base_array(){
		...
		//取得校長名稱，列出所有在職校長，順便檢查該校校長在職正確性。
		$sql_select = "select name from teacher_base as a,teacher_post as b where b.teach_title_id=1 and a.teacher_sn=b.teacher_sn and a.teach_condition=0";
        $recordSet=$CONN->Execute($sql_select);
		$ttt="";
		while (!$recordSet->EOF) {
		 $ttt.=$recordSet->fields["name"]." ";
         $recordSet->MoveNext();
		}
		$school['校長']=$ttt;
		
		//取得教務主任名稱，列出所有在職教務主任，順便檢查該校教務主任在職正確性。
		$sql_select = "select name from teacher_base as a,teacher_post as b where b.teach_title_id=2 and a.teacher_sn=b.teacher_sn and a.teach_condition=0";
        $recordSet=$CONN->Execute($sql_select);
        $ttt="";
		while (!$recordSet->EOF) {
		 $ttt.=$recordSet->fields["name"]." ";
		...
		//取得學務主任名稱，列出所有在職學務主任，順便檢查該校學務主任在職正確性。
		$sql_select = "select name from teacher_base as a,teacher_post as b where b.teach_title_id=3 and a.teacher_sn=b.teacher_sn and a.teach_condition=0";
        $recordSet=$CONN->Execute($sql_select);
        $ttt="";
		while (!$recordSet->EOF) {
		 $ttt.=$recordSet->fields["name"]." ";
		...
		//取得總務主任名稱，列出所有在職總務主任，順便檢查該校總務主任在職正確性。
		$sql_select = "select name from teacher_base as a,teacher_post as b where b.teach_title_id=4 and a.teacher_sn=b.teacher_sn and a.teach_condition=0";
        $recordSet=$CONN->Execute($sql_select);
        $ttt="";
		while (!$recordSet->EOF) {
		 $ttt.=$recordSet->fields["name"]." ";
