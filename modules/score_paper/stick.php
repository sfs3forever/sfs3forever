...
elseif($act=="dl_pdf" ){
	...
	foreach($all_stud_array as $student_sn => $stud_id){
		...
		//出缺席資料
		if($IS_JHORES==0) $temp3=get_abs_value($stud_id,$sel_year,$sel_seme,"標籤_成");
		else $temp3=get_abs_value($stud_id,$sel_year,$sel_seme,"種類");
		//缺席總日數
		if($IS_JHORES==0) $temp3['缺席總日數']=$temp3['事假_成']+$temp3['病假_成']+$temp3['曠課_成'];
		else $temp3['缺席總日數']=$temp3['事假']+$temp3['病假']+$temp3['曠課'];
		//總評與分數
...
function &score_paper_mainForm(){
	...
	$cols_options.="<option value='chc_2' $selected_chc_2>彰化縣版_九中高</option>";
	//台中市版-1
	...
	//$course_options.="<option value='事假' STYLE='background-color: #FBF292;  color: #F71CFF'>事假日數</option>\n";
        if($IS_JHORES==0) $course_options_arr['事假_成']='事假_成';
	else $course_options_arr['事假']='事假';
	//$course_options.="<option value='病假' STYLE='background-color: #FBF292;  color: #F71CFF'>病假日數</option>\n";
        if($IS_JHORES==0) $course_options_arr['病假_成']='病假_成';
	else $course_options_arr['病假']='病假';
	//$course_options.="<option value='曠課' STYLE='background-color: #FBF292;  color: #F71CFF'>曠課日數</option>\n";
        if($IS_JHORES==0) $course_options_arr['曠課_成']='曠課_成';
	else $course_options_arr['曠課']='曠課';
