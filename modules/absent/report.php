...
//取得本學期最多節次
	...
	for($i=1;$i<=$all_sections_max;$i++)
	{
      ...
      $ixp.="<input type='checkbox' name='sel[$i]' value='1' $checked onclick='this.form.submit();'>第 $i 節";
	}
...
//	$linkStr=(!empty($week_num))?"&week_num=$week_num":"";
	...
	<font color='blue'>$sel_year</font>學年度第<font color='blue'>$sel_seme</font>學期 勤惰週報表
	$week_select <br> $ixp <br>
	<input type='hidden' name='act' value='view'>
	<input type='hidden' name='this_date' value='$year-$month-$day'>
	<input type='submit' name='act' value='分班列印'><input type='submit' name='act' value='合併列印'><input type='submit' name='act' value='列印通知書(固定1~7節)'><input type='submit' name='act' value='列印通知書(自選節次)'></td></form></tr>
	</table>
