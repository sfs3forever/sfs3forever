...
if ($_POST[output_xml]) {
	...
	switch ($_POST['output_selected']) {
		case '99':
			...
			$sql="select class_id from school_class where year='{$year}' and semester='$seme' order by class_id limit 1";
			$res=$CONN->Execute($sql) or die($sql);
	...
	foreach ($class_row as $class) {
		...
		$query="select b.student_sn from stud_base a,stud_seme b where a.student_sn=b.student_sn and b.seme_class='$seme_class' and b.seme_year_seme='$c_curr_seme' and (a.stud_study_cond=0 or a.stud_study_cond=15) order by seme_num limit 1";
