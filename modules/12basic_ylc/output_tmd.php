...
if($_POST['act']){
	...
	switch($_POST['act']){
		...
		case 'EXCEL_SCORE':
			...
			$x->setBorderStyle(1);
			$x->addSheet($school_id);
			$x->items[0]=array('學生姓名','身分證號','出生年','出生月','出生日','經濟弱勢','偏遠小校','就近入學','獎勵記錄','出缺席記錄','無記過記錄','均衡學習','競賽成績','體適能');
			break;	
		case 'HTML_SCORE':
			$main="<table border='2' cellpadding='3' cellspacing='0' style='border-collapse: collapse' bordercolor='#111111' id='AutoNumber1'>
				<tr bgcolor='#ffcccc'><td>學生姓名</td><td>身分證號</td><td>出生年</td><td>出生月</td><td>出生日</td><td>經濟弱勢</td><td>偏遠小校</td><td>就近入學</td><td>獎勵記錄</td><td>出缺席記錄</td><td>無記過記錄</td><td>均衡學習</td><td>競賽成績</td><td>體適能</td>";
			break;	
	}
