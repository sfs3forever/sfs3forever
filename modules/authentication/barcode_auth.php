...
if($barcode AND $_POST['act']=='解析處理'){
	...
	$barcode=explode("\r\n",$barcode);
	$excuted="<BR>※ 前次條碼讀取匯入的記錄如下～<BR><BR>";
	$ask_items='';
	$error_info='<BR>※ 下面的認證記錄已經存在，系統未進行記錄更新！<br><br>';
	...
	foreach($barcode as $value){
		if($value) {
			...
			if($res->RecordCount()){
				...
				//}  else { $illegal_info.="<br>　　$value 您未獲授權進行 $curr_class_num - $stud_name --- {$subitem_array[$subitem_sn]['title']} 的認證！"; }		
...
$main="<table><form name='my_form' method='post' action='$_SERVER[PHP_SELF]'>
	...
	<BR>▲請掃瞄認證條碼：<BR><textarea rows='15' name='barcode' cols=33></textarea>
