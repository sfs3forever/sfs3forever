...
// $Id: board_c.php 5310 2009-01-10 07:57:56Z hami $
...
if ($key == "確定"){
	...
	$sql_insert = "insert into unit_c(bk_id,b_open_date,b_days,b_unit,b_title,b_name,b_sub,b_con,b_upload,b_url,b_own_id,b_post_time,b_is_intranet,teacher_sn,update_ip,act,b_kind)values ('$bk_id','$b_open_date','$b_days','$board_name ','$b_title ','$b_name ','$b_sub','$b_con ','$b_upload_name ','$b_url',{$_SESSION['session_log_id']},'$b_post_time','$b_is_intranet','$_SESSION[session_tea_sn]','{$_SERVER['REMOTE_ADDR']}','$a_b_id','$entry')";
	...
	if (!$error_flag)
		if($c_is_in=="")
			Header ("Location: etoe.php?unit=$unit&entry=$entry&m_id=$mm");
		else 
			Header ("Location: etoe.php?unit=$unit&entry=$entry&m_id=$mm");
