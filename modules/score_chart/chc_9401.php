...
$defult_txt="說明與注意事項：<BR>
分數 90～100 -> 優；分數 80～89 -> 甲；分數 70～79 -> 乙；分數 60～69 -> 丙；分數 0～59 -> 丁。
<BLOCKQUOTE>
1.本學期訂於 月 日結束，月 日開始放假。<BR>
2.假期中返校日定於 月 日。<BR>
3. 學年度 學期定於 月 日開學註冊，隨即上課，應繳交各項費用另行通知。<BR>
4.貴子弟本學期在校各項成績，均經評定，謹列表通知，即希察閱\。</BLOCKQUOTE>
<BR>
<CENTER>此致</CENTER>
<div style='margin-left:20pt;font-size:16pt'>貴家長</div>
...
if($_GET[year_seme]!='' && $_GET[grade]!='' ) {
	...
	if (file_exists($fname)) {
		$input_txt=implode('', file($fname));//讀檔成字串
		}
	else {
		
		mywrite($fname_1,$defult_txt);
	}
	$add_memo_file=chk_file($fname_1);
	$input_txt=implode('',file($add_memo_file));//讀檔成字串
}
...
// 選擇成績單類別
	if ($_POST[chart_kind]=="") $_POST[chart_kind]=($none_text*3)+3;
	$chked=array();
	$chked[$_POST[chart_kind]]="checked";
	$smarty->assign("none_text",$none_text);
	$smarty->assign("chked",$chked);
...
//是否無條件進位狀態
$smarty->display($template_dir."chc_940104.htm");
