...
//代課老師課表資料
	...
	<td  align='center'width=18%>代課日期</td>
	<td align='center'width=15%>結束日期或節次</td>
	<td align='center'width=17%>科目班級</td>
...
function teacher_all_class($sel_year="",$sel_seme="",$tsn=""){
	...
	for ($j=1;$j<=$sections;$j++){
		...
		for ($i=1;$i<=count($weekN); $i++) {
			...
			$subject_show=$kind[$k2].$subject_show.$kind[$k2];
	...
	//該班課表
	$main_class_list="
	<tr bgcolor='#FBDD47'><td colspan=6>『".$teacher_name."』授課總表 (★：兼課)</td></tr>
