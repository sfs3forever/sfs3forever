...
if($type_id){
	...
	while(list($student_sn,$stud_name,$curr_class_num,$stud_sex,$guardian_name,$guardian_hand_phone)=$recordSet->FetchRow()) {
		...
		if(!$guardian_hand_phone) {
			...
			$studentdata.="<td bgcolor='$stud_sex_color'>$class_name ($seme_num)$stud_name<br><font size=2 color='blue'><input type='checkbox' name='selected_MDN[]' value='$guardian_hand_phone'>[$guardian_name][$guardian_hand_phone]</font><input type='hidden' name='selected_MDN_name[$guardian_hand_phone]' value='$DestName'></td>";
		}
