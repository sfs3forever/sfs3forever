...
//echo "</pre>";
	...
	//***學生身分類別屬性
	$subkind_field=array(1=>'clan',2=>'area',3=>'memo',4=>'note');
	//將原住民族語認證通通清空
	$sql="UPDATE 12basic_tcntc SET language_certified=0 WHERE academic_year='$work_year' AND editable='1'";
	$rs=$CONN->Execute($sql) or user_error("讀取失敗！<br>$sql",256);
	//***

	//寫入資料表
	$sql="REPLACE INTO 12basic_kind SET year_seme='$work_year_seme',kind_data='$kind_data',disability_data='$disability_data',free_data='$free_data'";
	$res=$CONN->Execute($sql) or user_error("讀取失敗！<br>$sql",256);	

	//清除既有設定
	$sql="UPDATE 12basic_tcntc SET kind_id=0,disability_id=0,free_id=0,score_disadvantage=0 WHERE academic_year='$work_year' AND editable='1'";
	$res=$CONN->Execute($sql) or user_error("讀取失敗！<br>$sql",256);
	
	//抓取學生身分類別資料並決定其身分
	$sql="SELECT a.student_sn,a.editable,b.stud_kind,b.curr_class_num,b.stud_name FROM 12basic_tcntc a INNER JOIN stud_base b ON a.student_sn=b.student_sn WHERE a.academic_year='$work_year'";
	$res=$CONN->Execute($sql) or user_error("讀取失敗！<br>$sql",256);
	while(!$res->EOF){
		...
		if($editable){
			...
			foreach($my_kind_arr as $key=>$value){
				if($value){
					...
					//***增加判定原住民族語通過與否功能
					if($value == $native_id) {
						//native_language_sort   //native_language_text						
						$fieldname=$subkind_field[$native_language_sort];
			...
			//寫入資料表
			$sql="UPDATE 12basic_tcntc SET kind_id='$kind_id',disability_id='$disability_id',free_id='$free_id',score_disadvantage='$score_disadvantage',update_sn='$session_tea_sn' WHERE student_sn=$student_sn AND academic_year='$work_year' AND editable='1'";
...
$main="<form name='myform' method='post' action='$_SERVER[PHP_SELF]'><input type='hidden' name='edit_sn' value='$edit_sn'>$recent_semester $tool_icon
...
//抓取報名身分與低收失業對應表
$sql="SELECT * FROM `12basic_kind` WHERE year_seme='$work_year_seme'";
$rs=$CONN->Execute($sql) or user_error("讀取失敗！<br>$sql",256);
