...
if($_POST['act']=='清除所有教育會考成績'){
	$sql="UPDATE 12basic_tcntc SET score_exam_c=NULL,score_exam_m=NULL,score_exam_e=NULL,score_exam_s=NULL,score_exam_n=NULL,exam_memo=NULL WHERE academic_year=$work_year AND editable='1'";
...
$main.="<font color='red'>※請注意：試探與招生作業的教育會考表現積分計算不於SFS3處理，此處保留係為資料紀錄或日後模擬之用！</font><br>
<form name='myform' method='post' action='$_SERVER[PHP_SELF]'><input type='hidden' name='edit_sn' value='$edit_sn'>$recent_semester $class_list $tool_icon<table border='2' cellpadding='3' cellspacing='0' style='border-collapse: collapse' bordercolor='#111111' id='AutoNumber1' width='100%'>";
...
//echo "</pre>";
	...
	$studentdata="<tr align='center' bgcolor='#ff8888'><td width=80>學號</td><td width=50>座號</td><td width=120>姓名</td><td width=$pic_width>大頭照</td><td>國文</td><td>數學</td><td>英語</td><td>社會</td><td>自然</td><td>級分統計</td><td>寫作測驗</td><td>備註</td>";
	while(!$recordSet->EOF){
		...
		$score_exam_w=$exam_data[$student_sn]['score_exam_w'];		
		$memo=$exam_data[$student_sn]['exam_memo'];
		//級分統計
		$score=$score_exam_c+$score_exam_m+$score_exam_e+$score_exam_s+$score_exam_n;  //+$score_exam_w 
