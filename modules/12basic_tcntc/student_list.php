...
if($selected_stud AND $_POST['act']=='開列選擇的學生'){
	...
	$sql="REPLACE INTO 12basic_tcntc(academic_year,student_sn,update_sn) values $batch_value";
...
//刪除參加學生
if($_POST['student_sn']) {
	$sql="delete from 12basic_tcntc WHERE academic_year='$academic_year' and student_sn=".$_POST['student_sn'];
...
if($_POST['act']=='開列本學年所有的學生'){
	...
	$sql="INSERT INTO 12basic_tcntc(academic_year,student_sn,update_sn) values $batch_value";
	$res=$CONN->Execute($sql) or user_error("讀取失敗！<br>$sql",256);
};
