...
$memu_select="※我是 $stud_name ，本學期就讀班級： $curr_seme_class ，座號： $curr_seme_num 。<br>※我要檢視：";
...
//取得既有所選項目自我省思資料
if($menu){
	$act=$pos?"<input type='submit' value='儲存紀錄' name='go' onclick='return confirm(\"確定要\"+this.value+\"?\")'>":"";
...
switch($menu){
	...
	case '3-2':
		...
		} else $club_list.="<tr align='center'><td colspan=7 height=24>未發現社團活動紀錄！</td></tr>";
		$club_list.="</table>";
	...
	case '3-3':
		...
		$act=($pos and $race_result)?" <input type='submit' name='go' value='新增'>":"";
		//表格欄位抬頭
		...
		} else $race_list.="<tr align='center'><td colspan=8 height=24>未發現各項競賽成果紀錄！</td></tr>";
		$race_list.="</table>";
	...
	case '3-5':
		...
		<tr align='center' bgcolor='#ffcccc'><td>年級</td><td>學期</td><td>服務日期</td><td colspan=2>參加校內外公共服務學習事項及活動項目</td><td>分鐘數</td><td>主辦單位</td><td>登錄單位</td><td>自我省思</td>";
		...
		} else $service_list.="<tr align='center'><td colspan=9 height=24>未發現已認證的服務學習紀錄！</td></tr>";
		$service_list.="</table>";
		//統計表
	...
	case '3-6':	
		...
		$act=$pos?"<input type='submit' name='go' value='新增'>":"";
		//表格欄位抬頭
